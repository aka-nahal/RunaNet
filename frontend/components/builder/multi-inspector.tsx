  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <p className="text-xs font-semibold text-zinc-300">{count} tiles selected</p>
      ...
      {/* Selection summary */}
      <div className="rounded-lg bg-zinc-800/30 border border-zinc-800 p-2.5 space-y-1.5">
        <div className="flex flex-wrap gap-1">
          {selectedTiles.map((t) => {
            const info = TILE_TYPES.find((tt) => tt.value === t.tile_type)
            return (
              <span key={t.id} className="inline-flex items-center gap-1 rounded-md bg-zinc-800 px-1.5 py-0.5 text-[10px] text-zinc-400 border border-zinc-700">
                <span>{info?.icon ?? "?"}</span>
                #{t.id}
      ...
      <div className="space-y-1">
        <button
          ...
          className="flex w-full items-center justify-between text-[10px] font-semibold uppercase tracking-widest text-zinc-600 hover:text-zinc-400 py-1"
        >
          Bulk Edit
          ...
          <div className="space-y-2.5 pt-1">
            ...
            {allSameType && (
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  className="w-full rounded-md bg-zinc-800 px-2 py-1.5 text-sm text-zinc-200 outline-none border border-zinc-700 focus:border-blue-500/50"
                >
                  {TILE_TYPES.map((tt) => (
            ...
            <label className="flex flex-col gap-1">
              ...
              <input
                ...
                className="w-full rounded-md bg-zinc-800 px-2 py-1.5 text-sm text-zinc-200 outline-none border border-zinc-700 focus:border-blue-500/50 tabular-nums"
              />
            </label>
            ...
            <label className="flex items-center gap-2 text-xs text-zinc-400 cursor-pointer py-1">
              <input
                ...
                className="rounded border-zinc-600 bg-zinc-800 text-red-500"
              />
      ...
      {/* Tips */}
      <div className="rounded-lg bg-zinc-800/20 border border-zinc-800 p-2.5 text-[10px] text-zinc-600 space-y-1">
        <p>Use the <strong className="text-zinc-500">Align toolbar</strong> above the canvas.</p>
        <p><kbd className="rounded bg-zinc-800 px-1 text-zinc-500">↑↓←→</kbd> Nudge all selected tiles</p>
        <p><kbd className="rounded bg-zinc-800 px-1 text-zinc-500">Shift+Click</kbd> Toggle selection</p>
      </div>
    </div>
  )
}
...
function ActionBtn({ icon, label, onClick, danger }: { icon: string; label: string; onClick: () => void; danger?: boolean }) {
  return (
    <button onClick={onClick}
      className={`flex items-center gap-1.5 rounded-lg bg-zinc-800 py-2 px-2 text-[11px] border border-zinc-700 transition-colors ${
        danger
          ? "text-red-400 hover:bg-red-900/20 hover:border-red-500/30"
          : "text-zinc-300 hover:bg-zinc-700 hover:border-zinc-600"
      }`}>
